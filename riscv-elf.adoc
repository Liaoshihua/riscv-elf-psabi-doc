...
the `Itanium {Cpp} ABI` <<itanium-cxx-abi>>;
there are no RISC-V specific mangling rules.

See `Type encodings` section in `Itanium {Cpp} ABI`
for more detail on how to mangle types.

== ELF Object Files

The ELF object file format for RISC-V follows the
`Generic System V Application Binary Interface` <<gabi>>
...
`RISC-V Assembly Programmer's Manual` <<rv-asm>>.
